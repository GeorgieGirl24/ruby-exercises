...
class AnyPatternTest < Minitest::Test
  ...
  def test_has_at_least_one_alice
    ...
    has_alice = false
    names.each do |name|
      has_alice = true if name.start_with?("Alice")
    end
    assert has_alice
  end
  ...
  def test_no_alices
    ...
    has_alice = false
    names.each do |name|
      has_alice = false if name.start_with?("Alice")
    end
    refute has_alice
  end
  ...
  def test_has_a_multi_word_phrase
    ...
    phrases.each do |phrase|
      has_multi_word_phrase = true if phrase.count(" ") > 1 #|| (/!@.,%^&?/ > 1)
    end
  ...
  def test_no_monkeys
    ...
    animals.each do |animal|
      has_monkeys = false if animals.include?("monkey")
    end
    refute has_monkeys
  end
  ...
  def test_no_multiples_of_five
    ...
    numbers.each do |number|
      multiples_of_5 = false if numbers.include?(number % 5 == 0)
    end 
